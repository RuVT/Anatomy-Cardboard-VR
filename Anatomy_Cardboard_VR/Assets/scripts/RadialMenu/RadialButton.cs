...
public class RadialButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
	...
	public void GenerateRadialShape(float deg_i, float deg_f, float ri, float re){
			...
			for (int y = 0; y <= shape.height-1; y++)
			{
				...
				if (ri2 <= d2 && d2 <= re2)
				{
					...
					if (deg_i < deg_f) {
						...
						if ((deg_i <= angY && angY <= 360) || (0 <= angY && angY <= deg_f)) {
							shape.SetPixel (x, y, Color.white);
							if (x < minX)
								minX = x;
							if (y < minY)
		...
		Debug.Log ("Entro generate");
		File.WriteAllBytes (Application.dataPath + "/Resources/sprites/radioMenu/"+deg_i+"_"+deg_f+"_deg.png", data);
		#if UNITY_EDITOR
		AssetDatabase.ImportAsset ("Assets/Resources/sprites/radioMenu/" + deg_i + "_" + deg_f + "_deg.png");
		Texture2D tex = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/Resources/sprites/radioMenu/" + deg_i + "_" + deg_f + "_deg.png");
		SetImporterTextureFormat (tex, true);
		GetComponent<Image> ().sprite = AssetDatabase.LoadAssetAtPath<Sprite> ("Assets/Resources/sprites/radioMenu/" + deg_i + "_" + deg_f + "_deg.png");
		#endif
	}
...
//	}
	...
	#if UNITY_EDITOR
	public static void SetImporterTextureFormat(Texture2D texture, bool isReadeble){
		if (texture == null)
			return;
